...
Effects are WGSL fragment shaders paired with a JSON `.pfx` definition. Shaders have access to time, resolution, 46 audio features (7 frequency bands, beat detection, spectral shape, 13 MFCC timbral coefficients, 12 chroma pitch classes) — plus 15 reserved slots and three audio textures (waveform/spectrum/spectrogram) for loudness, key, downbeat, stereo and structure detectors landing in upcoming releases — up to 16 parameters, feedback from the previous frame, and a built-in library (noise, palette, SDF, tonemap).
