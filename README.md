...
Settings, presets, scenes and mappings all live in `~/.config/fosfora/` (and the equivalent on
macOS and Windows). Delete it to reset; nothing else on your system is touched. (Upgrading from
an older release? Your `~/.config/phosphor/` directory is moved over automatically on first
launch.)
