...
# ///
"""Dump key-path sidecars (e61 + gates) for a dataset's tune half (workstream Q3).

    bench/dump_key_sidecar.py bench/datasets/giantsteps_key [--jobs 4] [--all] [--force]

Runs the fosfora binary once per track with `FOSFORA_KEY_SIDECAR` set, writing
`bench/out/keysweep/<dataset>/<track_id>.jsonl` — the production front-end's exact
per-hop key inputs, for `bench/analyze_key.py --sweep` to replay the key back-end
against. Tune-half only by default (`is_tune` sha256 parity — selection must never
read holdout); `--all` dumps every track. The `--signal-dump` output itself is
discarded: sweeps recompute /key from the sidecar.
...
def main() -> int:
    ...
    out_dir = REPO / "bench" / "out" / "keysweep" / args.dataset.name
    ...
    print(f"keysweep: {len(todo)} tracks to dump -> {out_dir}", flush=True)

    def dump(job: tuple[str, Path, Path]) -> str | None:
        tid, audio, sidecar = job
        part = sidecar.with_suffix(".jsonl.part")
        env = os.environ | {"FOSFORA_KEY_SIDECAR": str(part)}
        r = subprocess.run(
