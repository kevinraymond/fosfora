#!/usr/bin/env python3
"""
Fosfora Bridge Base — shared websocket client for all bridge scripts.

This is the foundation for all Fosfora companion bridges. It handles
...
class FosforaBridge:
    ...
    def connect(self, retry_interval=2.0, max_retries=None):
        """
        Connect to Fosfora and send schema. Blocks until connected.
    ...
    def send_schema(self):
        """Send (or re-send) the current schema to Fosfora.
    ...
    def _drain(self):
        """Drain any inbound messages from Fosfora to prevent buffer backlog.

        Fosfora broadcasts state updates to all WS clients.  If the bridge
        never reads them the TCP receive buffer fills and Fosfora drops the
    ...
    def push(self, fields_dict):
        """
        Send a data frame to Fosfora.

        Args:
            fields_dict: dict mapping field_id -> float value.
                         Only include fields that changed, or all fields.
                         Fosfora holds last-known values for missing fields.
        """
        if not self._connected:
            return False

        # Drain inbound messages so Fosfora's send buffer never backs up
        self._drain()
    ...
    @staticmethod
    def common_args(description="Fosfora Bridge"):
        """
        ...
        parser.add_argument(
            ...
            help="Disable thumbnail preview in Fosfora binding matrix")
        parser.add_argument(
